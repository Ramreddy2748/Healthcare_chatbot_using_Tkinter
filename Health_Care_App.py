...
from tkinter import Tk, Label, Entry, Button, END, DISABLED, NORMAL, Menu, StringVar, messagebox, simpledialog
#Imports Library
import re
import webbrowser
...
class ChatApplication:
    ...
    def giveanswer(self, disease, ans):
    ...
    def _suffering_days(self):
        no_of_days = simpledialog.askstring("Please only respond in a number of days.", "Since how many days do you suffer? ")
        if no_of_days is None:
            self.msg_warning("Wrong Input", "Please respond only in number format. Do not cancel because it's important.")
            self._suffering_days()
        elif no_of_days.isnumeric() and int(no_of_days) > 0:
    ...
    def ask_box(self, desid):
        prompt = simpledialog.askstring(f"Please respond only with (Yes/No)", f"Are you suffering from a ' {desid} ' ? ")
        if prompt is None:
    ...
    def _get_name(self):
        ...
        self.window.configure(width=500, height=500, bg=self.BG_COLOR)

        # tiny divider
        line = Label(self.window, width=450, bg=self.BG_GRAY)
        line.place(relwidth=1, y=0, relheight=0.002)

        # name entry label
        self.name_label = Label(self.window, bg=self.BG_COLOR, fg=self.TEXT_COLOR,
                                 text="Please Enter Your Name", font=self.FONT_BOLD, pady=10)
        self.name_label.place(relwidth=1, y=57.3)

        # name entry box
        name_label = Label(self.window, bg=self.BG_COLOR, height=80)
        name_label.place(relheight=0.09, relwidth=1, y=105.9)
        self.name_entry = Entry(name_label, bg=self.BG_GRAY, fg="BLACK", font=self.FONT_BOLD)
        ...
        name_box_label = Label(self.window, bg=self.BG_COLOR, height=80)
        name_box_label.place(relheight=0.09, relwidth=1, y=150.9)
        name_send_button = Button(name_box_label, text="Submit", fg=self.TEXT_COLOR, font=self.FONT_BOLD, width=20,
                                  bg="RED",
                                  command=lambda: self.get_name_after_click(None))
        name_send_button.place(relheight=1, relwidth=0.24, x=190)

        # Age label
        self.age_label = Label(self.window, bg=self.BG_COLOR, fg=self.TEXT_COLOR,
                                text="Please Enter Your Age", font=self.FONT_BOLD, pady=10)
        self.age_label.place(relwidth=1, y=207.3)

        # Age entry box
        age_label = Label(self.window, bg=self.BG_COLOR, height=80)
        age_label.place(relheight=0.09, relwidth=1, y=255.9)
        self.age_entry = Entry(age_label, bg=self.BG_GRAY, fg="BLACK", font=self.FONT_BOLD)
        self.age_entry.place(relheight=0.85, relwidth=0.74, x=65)
        self.age_entry.bind("<Return>", self.get_age_after_click)

        # save age button label
        age_box_label = Label(self.window, bg=self.BG_COLOR, height=80)
        age_box_label.place(relheight=0.09, relwidth=1, y=300.9)
        age_send_button = Button(age_box_label, text="Submit", fg=self.TEXT_COLOR, font=self.FONT_BOLD, width=20,
                                 bg="RED",
                                 command=lambda: self.get_age_after_click(None))
        age_send_button.place(relheight=1, relwidth=0.24, x=190)
    ...
    def get_age_after_click(self, event):
        age = self.age_entry.get()
        #self.age_entry.delete(0, END)
        self.msg_showinfo(f"Your Age is {age}", "Your Age has been recorded")
    ...
    def coming_msg(self):
        ...
        msg1= f"Hey! {good_name} , Are you not feeling well?"
        msg2="Please tell me what symptoms here are some examples:\n --> fever\n --> cold\n --> cough\n --> headache\n --> stomach_pain\n --> abdominal_pain\n --> dehydration\n --> swelling\n --> acidity\n --> internal_itching\n --> sneezing\n --> vomiting\n --> anxiety , etc \n Note: Please use underscore (  _  ) in place of spacing in the name of disease.\n\n"
